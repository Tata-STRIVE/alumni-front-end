...
const Header = () => {
    ...
    const navLinkClasses = ({ isActive }) => 
        `px-3 py-2 rounded-md text-sm font-medium ${isActive ? 'bg-strive-orange text-white' : 'text-gray-300 hover:bg-blue-800 hover:text-white'} transition-colors`;
                    ...
                    <div className="flex items-center">
                        ...
                        <div className="hidden md:block md:ml-10">
                            <div className="flex items-baseline space-x-4">
                                <NavLink to="/dashboard" className={navLinkClasses} end>{t('header.my_hub')}</NavLink>
            ...
            {isMobileMenuOpen && (
                <div className="md:hidden">
                    <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                        <NavLink to="/dashboard" className={mobileNavLinkClasses} onClick={() => setIsMobileMenuOpen(false)} end>{t('header.my_hub')}</NavLink>
                        <NavLink to="/" className={mobileNavLinkClasses} onClick={() => setIsMobileMenuOpen(false)}>{t('header.home')}</NavLink>
                        ...
                        <NavLink to="/dashboard/profile" className={mobileNavLinkClasses} onClick={() => setIsMobileMenuOpen(false)}>My Profile</NavLink>
                        <NavLink to="/dashboard/connections" className={mobileNavLinkClasses} onClick={() => setIsMobileMenuOpen(false)}>My Connections</NavLink>
                    </div>
                    <div className="pt-4 pb-3 border-t border-blue-800 px-5">
